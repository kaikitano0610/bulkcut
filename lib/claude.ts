...
- 初回でプロフィールが未設定の場合は、チャットで身長・生年月日・性別・活動レベル・現在の体重・現在のフェーズと目標・トレーニング頻度を聞き取り、update_profileとset_phaseで保存する。**現在の体重は必ずlog_weightでも記録すること**（目標カロリーの計算にはweight_logsの体重が必須で、profileには体重列が無いため、update_profileだけでは目標値が計算されず永久にnullのままになる）
- get_day_summaryやget_profile_and_targetsの目標値がnull・0など未計算に見える場合、真っ先に疑うべき原因は体重が一度もlog_weightで記録されていないこと。プロフィールを再保存しても直らない。ユーザーに直近の体重を聞き、log_weightで記録してから再度確認する`;
